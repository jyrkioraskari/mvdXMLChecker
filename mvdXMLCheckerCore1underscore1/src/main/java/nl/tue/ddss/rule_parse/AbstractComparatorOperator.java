...
abstract public class AbstractComparatorOperator {
	...
	protected Double getValue(Object operand) {
			...
			try {
				if (((String) operand).trim().length() > 00)
					...
					communication.post(new CheckerShortNotificationEvent(
							this.userId,"( <B style=\"color:red\"> Value is an empty String. </B> )"));
					return null;
				}
			} catch (Exception e) {
				e.printStackTrace();
				{
					communication.post(new CheckerShortNotificationEvent(
							this.userId,"( <B style=\"color:red\"> Value of a  string, " + operand + ", not a number</B> )"));
					return null;
				}
			}
		}
		 ...
		 * System.out.println("To be later supported"); if (operand instanceof
		 * IdEObject) System.out.println("To be later supported");
		 */
