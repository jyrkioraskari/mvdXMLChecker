...
public class IfcMVDConstraintChecker {
	...
	public IssueReport checkModel(IfcModelInterface ifcModel, File ifcfile)
		...
		for (MVDConceptConstraint constraint : constraints) {
			...
			communication.post(new CheckerElementApplicabilityNotificationEvent(
					this.userId,"Applicability Operator: " + constraint.getApplicability_operator()));
			for (TemplateRule applicability_templateRule : constraint.getApplicability_templateRules()) {
				communication.post(new CheckerElementApplicabilityNotificationEvent(
						this.userId,"Applicability rule: " + applicability_templateRule.getParameters()));
			}

			communication.post(new CheckerInfoEvent(this.userId,"Concept validity Operator: ", constraint.getConcept_operator()));
			for (TemplateRule templateRule : constraint.getConcept_templateRules()) {
				communication.post(
						new CheckerElementValidityNotificationEvent(this.userId,"Concept rule: " + templateRule.getParameters()));
			}
	...
	private boolean elementApplicability_check(MVDConceptConstraint constraint, Object ifcObject) {
			...
			if (constraint.getApplicability_operator().toLowerCase().trim().equals("or")) {
				for (TemplateRule applicability_templateRule : constraint.getApplicability_templateRules()) {
					for (int i = 0; i < applicability_hashMaps.size(); i++) {
						communication.post(new CheckerElementApplicabilityNotificationEvent(
								this.userId,"<BR>Tested applicability value set: " + ai++ + ""));
						...
						if (result == true) {
							applicable = true;
							communication.post(new CheckerElementApplicabilityNotificationEvent(this.userId,"   <BR>Test passed."));
							break;
						}
					}
				}
		...
		if (!applicable) {
			...
			if (this.ifcversion == IfcVersion.IFC2x3)
				...
				communication.post(new CheckerElementApplicabilityNotificationEvent(
						this.userId,"<B>" + ((org.bimserver.models.ifc4.IfcRoot) ifcObject).getGlobalId() + " is applicable.</B>"));
		}
		return true;
	}

	private void elementValidity_check(IssueReport issuereport, MVDConceptConstraint constraint, Object ifcObject,
			List<HashMap<AbstractRule, ObjectToValue>> concept_hashMaps, String comment) {
		boolean valid = false;
		int ci = 0;
			...
			if (constraint.getConcept_operator().toLowerCase().trim().equals("or")) {
				for (TemplateRule templateRule : constraint.getConcept_templateRules()) {
					for (int i = 0; i < concept_hashMaps.size(); i++) {
						communication.post(new CheckerInfoEvent(this.userId,"<p style=\"margin-left:10%;\"><BR>Concept value set: ",
								"" + ci++));
						...
						if (result == true) {
							valid = true;
							communication.post(new CheckerElementValidityNotificationEvent(
									this.userId,"   <BR>Value set accepted against OR TemplateRules,"));
							break;
						}
					}
				}
				...
				for (TemplateRule templateRule : constraint.getConcept_templateRules()) {
					...
					for (int i = 0; i < concept_hashMaps.size(); i++) {
						...
						if (result == true) {
							template_validity = true;
							communication.post(new CheckerElementValidityNotificationEvent(
									this.userId,"   <BR>Value set accepted against TemplateRules,"));
							break;
						}
						communication.post(new CheckerShortNotificationEvent(this.userId,"</p>"));
					}
		...
		if (comment.length() > 0) {
			...
			case IFC2x3:
				if (ifcObject instanceof org.bimserver.models.ifc2x3tc1.IfcProduct) {
					...
					communication.post(new CheckerIssueEvent(
			...
			case IFC4:
				...
				if (ifcObject instanceof org.bimserver.models.ifc4.IfcProduct) {
					...
					communication.post(new CheckerIssueEvent(this.userId,"<B>Other than IfcProduct issue</B><BR> guid="
							+ ((org.bimserver.models.ifc4.IfcRoot) ifcObject).getGlobalId(), "<BR>" + comment));
				}
				break;
			default:
