		...
		if (rightOperand instanceof String) {
			if (((String) rightOperand).startsWith("reg")) {
				...
				if (leftOperand instanceof String) {
					...
					// System.out.println("No matching type 1 " + leftOperand + " <> " +
					// rightOperand);
					result = false;
				}
			} else {
				if (leftOperand instanceof String) {
					if (leftOperand.equals(rightOperand)) {
						...
						if (((String) leftOperand).toLowerCase().equals(rightOperand.toString().toLowerCase())) {
							...
							// System.out.println("No matching type 2 " + leftOperand + " <> " +
							// rightOperand);
							result = false;
						}
					}
					...
					if (typeName.equals(rightOperand)) {
						...
						// System.out.println("No matching type 3" + leftOperand + " <> " +
						// rightOperand);
						result = false;
					}
					...
					for (int i = 0; i < valueList.size(); i++) {
						if (valueList.get(i) instanceof String) {
							...
							if (!typeName.equals(rightOperand) && i == valueList.size() - 1) {
								// System.out.println("Not matching type 4 " + leftOperand + " <> " +
								// rightOperand);
								result = false;
							}
						} else {
							// System.out.println("Not matching type 5 " + leftOperand + " <> " +
							// rightOperand);
							result = false;
						}
					}
				} else {
					// System.out.println("Not matching type 6 " + leftOperand + " <> " +
					// rightOperand);
					result = false;
				}
			}
			...
			if (leftOperand instanceof Double) {
				if (leftOperand.equals(rightOperand))
					...
					// System.out.println("Not matching type 7 " + leftOperand + " <> " +
					// rightOperand);
					result = false;
				}
				...
				if (doub.equals(rightOperand))
					...
					// System.out.println("Not matching type 8 " + leftOperand + " <> " +
					// rightOperand);
					result = false;
				}
			} else {
			...
			if (leftOperand instanceof IdEObject) {
				...
				result = false;
				// System.out.println("Not matching type 9 " + leftOperand + " <> " +
				// rightOperand);
			}
			...
			// System.out.println("To be later supported");
			result = false;
			// System.out.println("Matching type 1 bug " + leftOperand + " <> " +
			// rightOperand);
		} else if (rightOperand instanceof Boolean) {
			if (leftOperand instanceof Boolean) {
				result = leftOperand.equals(rightOperand);
			} else {
				// System.out.println("No matching type 10 bug " + leftOperand + " <> " +
				// rightOperand);
				result = false;
			}
		}
		if (result == false) {
			communication.post(new CheckerShortNotificationEvent(this.userId,
					"( <B style=\"color:red\"> \"" + leftOperand + "\" <> \"" + rightOperand + "\"</B> )"));
		} else {
			communication.post(new CheckerShortNotificationEvent(this.userId,
					"( <B style=\"color:green\"> \"" + leftOperand + "\" = \"" + rightOperand + "\"</B> )"));
		}
		return result;
	}
}