...
public class IfcHashMapBuilder {
	...
	public IfcHashMapBuilder(String userId, Object ifcObject, List<AttributeRule> attributeRules,
		...
		try {
			List<HashMap<AttributeRule, Object>> hMs = buildValueMaps(ifcObject, attributeRules, initialHMs);
	...
	private List<HashMap<AttributeRule, Object>> buildValueMaps(Object ifcObject,
			List<AttributeRule> mvdXML_template_attributeRules, List<HashMap<AttributeRule, Object>> hashMaps)
		...
		for (AttributeRule mvdXML_attributeRule : mvdXML_template_attributeRules) {
			...
			if (IFC_value == null) {
				...
				case 1:
					...
					break;
				default:
					List<HashMap<AttributeRule, Object>> hMs = copyHashMaps(hashMaps);
					for (int i = 0; i < IFC_valueList.size(); i++) {
						if (i == 0) {
							...
							List<HashMap<AttributeRule, Object>> hashMapList = copyHashMaps(hMs);
							mvdXML_entityRules = mvdXML_attributeRule.getEntityRules().getEntityRule();
							for (EntityRule mvdXML_entityRule : mvdXML_entityRules) {
								if (mvdXML_entityRule.getAttributeRules() != null && isInstance(IFC_valueList.get(i), this.ifc_class_base + mvdXML_entityRule.getEntityName())
										) {
	...
	private List<HashMap<AttributeRule, Object>> copyHashMaps(
			List<HashMap<AttributeRule, Object>> hashMaps) {
		List<HashMap<AttributeRule, Object>> hashMapList = new ArrayList<HashMap<AttributeRule, Object>>();
		for (HashMap<AttributeRule, Object> hashMap : hashMaps) {
